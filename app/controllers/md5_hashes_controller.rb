...
class Md5HashesController < ApplicationController
	def index
		@md5hashes = Md5Hash.paginate(:page => params[:page]).order(:hex_hash)
	...
	def upload_passwords
		...
		if upload != nil
			...
			count = Md5Hash.import_passwords_from_file upload[:datafile].path

			t2 = Time.now.to_f
			flash[:notice] = count.to_s + " password submitted. " + (t2-t1).to_s + " seconds."
			redirect_to md5_hashes_path
		else
			flash[:error] = "No password file selected"
			render 'new'
		end
	end
  ...
  def unsolved_b #slightly faster and doesnt have to render
    file = Tempfile.new("unsolved")
    Rails.logger.debug(file)
    Md5Hash.export_unsolved_hashes_to_file(file)
    send_file file.path, :type => 'text/plain', :filename => "unsolved.txt"
  end
    ...
    Rails.logger.debug("!!!!! Export time: " + (t2 - t1).to_s + " seconds")
	end
